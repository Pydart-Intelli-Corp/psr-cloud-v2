...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!otpData) {
      return createErrorResponse('OTP session not found. Please request a new OTP.', 404);
    }

    // Check if OTP is expired
    if (new Date() > otpData.expiry) {
      global.otpStore.delete(email.toLowerCase());
      return createErrorResponse('OTP has expired. Please request a new OTP.', 400);
    }

    // Verify OTP
    if (otpData.otp !== otp.trim()) {
      return createErrorResponse('Invalid OTP. Please check and try again.', 400);
    }
